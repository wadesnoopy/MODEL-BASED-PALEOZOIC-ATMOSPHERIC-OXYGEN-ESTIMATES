...
input <- read.csv("GEOCARB_input_summaries_20%_sulfate_better.csv")
...
for (i in 1:resampleN) {
  ...
  for (j in 1:ageN) {
    ...
    # For the inverse method that only solves d13C, the following scaling factors are all 1
    cc <- 1
    oo <- 1
    pp <- 1
    ss <- 1
    ...
    # Below is the inverse method to calculate the required Fbg
    
    # start inverse method
    
    Fbg <- Fwgy+Fwga+Fmg + (15/8)*(Fwpy+Fwpa+Fmp) - 15/8 * Fbp + smO2[j,i]/Dt
    
    s_dCcar <- (Fbg * CAPd13C + dlcy*Fwcy + dlca*Fwca + dlgy*Fwgy + dlga*Fwga + dlca*Fmc + dlga*Fmg)/ (Fwcy + Fwca + Fwgy + Fwga + Fmc + Fmg)
    ...
    dlgy <- dlgy+((s_dCcar-dlgy-CAPd13C)*Fbg/Gy)*Dt  #d13C of young organic matter
    dlga <- dlga+(Fyog*(dlgy-dlga)/Ga[j,i])*Dt  #d13C of old organic matter
    dlcy <- dlcy+((s_dCcar-dlcy)*Fbc/Cy)*Dt  #d13C value of young crustal carbonate carbon
...
if (Godderis == FALSE) {
  ...
  file_out <- paste0('GEOCARB_20%_sulfate_forced_d13C_better_revision_monte_fail_test_', scenario, '.csv')
}
